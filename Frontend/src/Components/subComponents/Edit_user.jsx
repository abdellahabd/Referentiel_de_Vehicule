...
import { Postadduser, fetchOne, updateuser } from "../../API/User.js";
...
function EditUse() {
  ...
  const handelFormSubmit = async (e) => {
    try {
      ...
      // console.log(checked);
      // console.log(checkeCotrat);
      // console.log(checkeUser);
      // console.log(checkeModele);
      // console.log(checkeChf);
      // console.log(checkeRelation);
  ...
  useEffect(() => {
    fetchOne(id).then((Olduser) => {
      setCheckeModele(Olduser.Accesses.Modele);
      setCheckeUser(Olduser.Accesses.user);
      setChecked(Olduser.Accesses.car);
      setcheckeChf(Olduser.Accesses.chf);
      setcheckeRelation(Olduser.Accesses.relation);
      setcheckeCotrat(Olduser.Accesses.Contrat);
      setuser({
        email: Olduser.user.email,
        name: Olduser.user.name,
        StructerCodeS: `${Olduser.user.StructerCodeS}`,
        password: Olduser.user.password,
        prenom: Olduser.user.prenom,
      });
      setloading(false);
    });
    ...
    >
      ...
      <Card className="bg-[#ffffff]  m-auto w-11/12  items-center flex flex-col">
        <CardContent className=" bg-[#ffffff] flex flex-col  ">
          <h1 className="text-3xl text-gray-800 m-auto">
          ...
          <div className="flex flex-col  m-auto gap-4 my-10 w-[26rem] ">
            ...
            <Autocomplete
              ...
              // value={selectvalue}
              defaultValue={{
                label: `${user.StructerCodeS}`,
                id: user.StructerCodeS,
              }}
              ...
              onSelect={(e) => {
                setuser({ ...user, str: e.target.value.split("(")[0].trim() });
              }}
              renderInput={(params) => (
